...
def distanza_da_acqua_km(lat, lon, tipo_acqua, raggio_ricerca_m=20000):
    """Overpass API (OpenStreetMap, gratuito): trova il tratto di costa marina o di riva di lago
    più vicino alle coordinate date, entro un raggio di ricerca, e ne stima la distanza reale.
    tipo_acqua: 'mare' o 'lago'. Ritorna la distanza in km, o None se non trovato nel raggio o in
    caso di errore di rete. La distanza è approssimata al vertice più vicino della geometria
    restituita da OpenStreetMap (non al punto esatto sul segmento), sufficientemente precisa per
    un filtro 'vicino al mare/lago' ma non per calcoli di precisione.
    ATTENZIONE: una chiamata per ogni hotel può essere lenta su tante zone/risultati; per questo
    è messa in cache e va posizionata come ULTIMO filtro nella catena (si valuta solo sugli hotel
    che hanno già superato gli altri filtri, più economici)."""
...
if check_password():
    ...
    with st.sidebar.expander("3️⃣ Filtri di ricerca"):
        ...
        st.caption(
            ...
            "contemporaneamente in più regioni con mari/laghi diversi tra loro."
        )
        ...
        if vicino_mare_attivo:
            distanza_mare_max_km = st.slider(
                "Distanza massima dalla costa (km):", min_value=0.5, max_value=20.0, value=5.0, step=0.5
            )
        ...
        if vicino_lago_attivo:
            distanza_lago_max_km = st.slider(
                "Distanza massima dal lago (km):", min_value=0.5, max_value=20.0, value=5.0, step=0.5
            )
        if vicino_mare_attivo or vicino_lago_attivo:
            st.caption(
                "⚠️ Interroga OpenStreetMap per ogni hotel superstite agli altri filtri: con molti "
                "risultati può rallentare visibilmente il caricamento della tabella (i risultati "
        ...
        if punto_riferimento_nome.strip():
            distanza_punto_max_km = st.slider(
                "Distanza massima dal punto (km, linea d'aria):", min_value=1.0, max_value=100.0, value=15.0, step=1.0
            )
    ...
    if sorgente_dati is None:
        ...
        # Geocodifica il punto di riferimento manuale UNA SOLA VOLTA (è cache-ata, ma evitiamo comunque
        # di richiamarla per ogni hotel del ciclo qui sotto).
        ...
        for dest in sorgente_dati:
            ...
            # Distanza da un punto di riferimento manuale: linea d'aria, economica, va prima del
            # filtro mare/lago che invece interroga una API esterna per ogni hotel.
            if rispetta_filtri and punto_riferimento_coord is not None:
                distanza_punto_km = _haversine_km(
                    dest["lat"], dest["lon"], punto_riferimento_coord[0], punto_riferimento_coord[1]
                )
                rispetta_filtri = distanza_punto_km <= distanza_punto_max_km
            # Distanza da mare/lago: la più costosa (una chiamata Overpass per hotel, seppur cache-ata),
            # per questo va per ultima nella catena — si valuta solo su chi ha già superato tutto il resto.
            if rispetta_filtri and vicino_mare_attivo:
                distanza_mare_km = distanza_da_acqua_km(round(dest["lat"], 4), round(dest["lon"], 4), "mare")
                rispetta_filtri = distanza_mare_km is not None and distanza_mare_km <= distanza_mare_max_km
            if rispetta_filtri and vicino_lago_attivo:
                distanza_lago_km = distanza_da_acqua_km(round(dest["lat"], 4), round(dest["lon"], 4), "lago")
                rispetta_filtri = distanza_lago_km is not None and distanza_lago_km <= distanza_lago_max_km
            if rispetta_filtri:
