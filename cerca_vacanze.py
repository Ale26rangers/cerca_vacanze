...
def punto_acqua_piu_vicino(lat, lon, tipo_acqua, raggio_ricerca_m=20000):
    ...
    nel raggio o in caso di errore di rete."""
...
def distanza_acqua_auto(lat, lon, tipo_acqua):
    ...
    se non è stato trovato alcun tratto di costa/lago nel raggio di ricerca.
    ATTENZIONE: due chiamate di rete per ogni hotel (Overpass + OSRM), seppur cache-ate; per
    questo va posizionato come ULTIMO filtro nella catena (si valuta solo sugli hotel che hanno
    già superato gli altri filtri, più economici)."""
    ...
    if sorgente_dati is None:
        ...
        for dest in sorgente_dati:
            ...
            if rispetta_filtri and punto_riferimento_coord is not None:
                ...
                elif unita_punto == "Km reali in auto":
                    rispetta_filtri = info_punto["km"] <= distanza_punto_max_km
                else:
                    rispetta_filtri = (info_punto["ore"] * 60) <= tempo_punto_max_min
            ...
            if rispetta_filtri and vicino_mare_attivo:
                ...
                if info_mare is None:
                    rispetta_filtri = False
                elif unita_mare == "Km reali in auto":
                    rispetta_filtri = info_mare["km"] <= distanza_mare_max_km
                else:
                    rispetta_filtri = (info_mare["ore"] * 60) <= tempo_mare_max_min
            if rispetta_filtri and vicino_lago_attivo:
                ...
                if info_lago is None:
                    rispetta_filtri = False
                elif unita_lago == "Km reali in auto":
                    rispetta_filtri = info_lago["km"] <= distanza_lago_max_km
                else:
                    rispetta_filtri = (info_lago["ore"] * 60) <= tempo_lago_max_min
            if rispetta_filtri:
