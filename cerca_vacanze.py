...
def cerca_hotel_zona(zona_query, check_in_str, check_out_str, adulti, bambini, api_key, max_risultati=10, filtri_extra=None):
...
def esegui_ricerca_geografica(origine_lat, origine_lon, zone, check_in, check_out, adulti, bambini, api_key, filtri_extra=None, max_risultati=10):
    """
    ...
    for zona in zone:
        proprieta, errore = cerca_hotel_zona(
            ...
            max_risultati=max_risultati, filtri_extra=filtri_extra,
        )
        if errore:
...
if check_password():
    ...
    st.sidebar.caption("↑ Questo limite è sempre istantaneo: le ore di guida sono già note per ogni hotel trovato.")

    st.sidebar.markdown("**🎯 Filtri di ricerca (lato server):**")
    st.sidebar.warning(
    ...
    piscina_obbligatoria = st.sidebar.checkbox("Piscina", value=True)
    wifi_obbligatorio = st.sidebar.checkbox("WiFi gratuito", value=False)
    colazione_obbligatoria = st.sidebar.checkbox("Colazione inclusa", value=False)
    parcheggio_obbligatorio = st.sidebar.checkbox("Parcheggio gratuito", value=False)
    animali_obbligatorio = st.sidebar.checkbox("Animali ammessi", value=False)
    cancellazione_obbligatoria = st.sidebar.checkbox("Cancellazione gratuita", value=False)
    categoria_minima = st.sidebar.select_slider(
        "Categoria minima (stelle):", options=[0, 1, 2, 3, 4, 5], value=0,
        help="0 = nessun filtro."
    )
    valutazione_minima = st.sidebar.select_slider(
    ...
    with st.sidebar.expander("➕ Altri servizi (lato server)"):
    ...
    st.sidebar.markdown("**🔍 Rifiniture locali (istantanee):**")
    st.sidebar.caption(
    ...
    vicino_sci_obbligatorio = st.sidebar.checkbox("Vicino agli impianti di risalita", value=False)
    deposito_sci_obbligatorio = st.sidebar.checkbox("Deposito / noleggio sci", value=False)
    scuola_sci_obbligatoria = st.sidebar.checkbox("Scuola sci", value=False)
    ...
    if usa_ricerca_geografica:
        with st.sidebar.expander("🔑 Dove trovo la mia SerpApi API Key?"):
            st.markdown(
        ...
        if serpapi_key:
            ...
            if errore_quota:
                st.sidebar.caption(f"⚠️ Impossibile leggere la quota residua: {errore_quota}")
            elif quota:
                ...
                if rimanenti is not None:
                    ...
                    if totale_disponibile <= 20:
                        st.sidebar.error(f"🔴 Solo {totale_disponibile} ricerche rimaste (piano + extra)!")
                    elif totale_disponibile <= 50:
                        st.sidebar.warning(f"🟠 {totale_disponibile} ricerche rimaste (piano + extra).")
                    else:
                        st.sidebar.caption(f"🟢 {totale_disponibile} ricerche rimaste (piano + extra).")
                if extra:
                    st.sidebar.caption(f"↳ di cui {extra} crediti extra.")
        ...
        max_hotel_per_zona = st.sidebar.slider(
        ...
        ordinamento_scelto = st.sidebar.selectbox(
        ...
        st.sidebar.caption(
            "⚠️ Cambiare '🎯 Filtri di ricerca' **non** aggiorna da solo i risultati mostrati (vedi nota "
            "sopra): serve premere di nuovo questo pulsante. Cambiare invece 'Limiti di viaggio' o "
            "'🔍 Rifiniture locali' aggiorna la tabella all'istante, senza consumare una nuova ricerca."
        )
    ...
    filtri_gia_applicati_lato_server = sorgente_dati is not destinazioni_mock
    ...
    if not df.empty:
        ...
        colonne_mostrate = ["nome", "regione", "categoria", "valutazione_testo", "servizi", "ore_guida", "prezzo_notte_chf", "fonte_prezzo", "costo_benzina_chf", "costo_pedaggio_chf", "costo_vignetta_chf", "budget_globale_chf"]
        formati = {
